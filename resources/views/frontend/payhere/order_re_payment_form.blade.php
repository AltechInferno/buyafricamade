    ...
    <form style="display: none" method="POST" action="{{ \App\Utility\PayhereUtility::get_action_url() }}" id="payhere-checkout-form">
        ...
        <input type="hidden" name="return_url" value="{{ route('payhere.order_re_payment.return') }}">
        <input type="hidden" name="cancel_url" value="{{ route('payhere.order_re_payment.cancel') }}">
        <input type="hidden" name="notify_url" value="{{ route('payhere.order_re_payment.notify') }}">
